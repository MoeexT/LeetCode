...
//        bfs(root, sb);
    ...
    private void bfs(TreeNode root, StringBuilder sb) {
        ...
        do {
            ...
            for (int i = 0; i < size; i++) {
                TreeNode node = queue.poll();
                if (node != null) {
                    notEmpty = notEmpty || node.left != null || node.right != null;
    ...
    private TreeNode generate(String[] data, int index) {
        if (index >= data.length) return null;
        String val = data[index];
        if (!val.equals("n")) {
            TreeNode node = new TreeNode(Integer.parseInt(val));
            node.left = generate(data, index << 1);
            node.right = generate(data, (index << 1) + 1);
            return node;
        } else
            return null;
    }
